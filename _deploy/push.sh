...
# Сжатие стилей перед отправкой: на сервер должен уезжать свежий style.min.css.
# Тема сама решает, какой файл подключить — сжатый берётся, только если он не
# старше исходного, поэтому забытая сборка не сломает сайт, а просто вернёт
# несжатый вариант.
node "$(dirname "$0")/../_tools/minify-css.mjs" || echo "  сжатие стилей пропущено"
cd "$SRC" || exit 1
