...
<footer class="ftr" id="contacts">
  <div class="wrap">
    ...
    <?php
    /* Обёртка нужна ради вертикального разделителя: он идёт по середине и
       должен пройти через ВСЕ двухколоночные блоки — карточки, призывы и
       контакты — и упереться в последнюю горизонталь. Раньше он висел на
       карточках и обрывался на первой же линии. Нижняя строка с копирайтом
       осталась снаружи: она одноколоночная, делить там нечего.
       Верхний и нижний отступы схлопываются с крайними детьми, поэтому
       обёртка сама встаёт ровно от карточек до последней линии. */
    ?>
