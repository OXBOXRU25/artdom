...
/* Серый фон под шапкой и белый лист ниже — тот же приём, что на контактах.
   Включается классом на <body>: сам по себе .sheet задаёт белый лист, а
   серым его подложку красит именно этот класс. Без него полоса под шапкой
   оставалась белой, и перехода не было видно вовсе. */
add_filter(
	'body_class',
