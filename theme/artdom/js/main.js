...
(function () {
  ...
  /* Показываем сразу по окончании загрузки, без прежней задержки в 600 мс.
     Задержка ставилась, чтобы плашка не влезла в замер LCP, — но LCP держит
     заголовок первого экрана площадью 70 тысяч, плашка втрое меньше и
     кандидатом не станет. Зато её поздний выезд оказывался ПОСЛЕДНИМ
     изменением экрана: Lighthouse считал страницу «ещё рисующейся» до
     3473 мс, и Speed Index падал с 1.5 до 4.2 при неизменном LCP. */
  if (document.readyState === "complete") {
    показать();
  } else {
    window.addEventListener("load", показать);
  }
  ...
  /* Один обработчик на всю плашку вместо трёх отдельных: кнопок стало
     четыре, и каждая новая иначе требовала бы своей подписки. */
  function ответить(выбор) {
    записать(выбор);
    if (выбор === "yes") согласие();
    убрать();
  }

  плашка.addEventListener("click", function (событие) {
    var кнопка = событие.target.closest("button");
    if (!кнопка) return;

    /* «Настроить» — единственная кнопка, которая не закрывает плашку:
       она разворачивает список категорий, и капсула становится карточкой.
       Состояние держим атрибутом на самой плашке: по нему переключается
       раскладка в стилях. */
    if (кнопка.hasAttribute("data-cookie-manage")) {
      var панель = плашка.querySelector("[data-cookie-panel]");
      if (!панель) return;
      var открываем = панель.hidden;
      панель.hidden = !открываем;
      плашка.setAttribute("data-managing", открываем ? "true" : "false");
      кнопка.setAttribute("aria-expanded", открываем ? "true" : "false");
      return;
    }

    /* «Сохранить выбор» читает флажок аналитики. Флажка нет вовсе, когда
       счётчик не подключён, — тогда сохранять нечего, кроме отказа от
       необязательного, и это равно «Отклонить». */
    if (кнопка.hasAttribute("data-cookie-save")) {
      var флажок = плашка.querySelector("[data-cookie-stat]");
      ответить(флажок && флажок.checked ? "yes" : "no");
      return;
    }

    if (кнопка.hasAttribute("data-cookie-ok")) { ответить("yes"); return; }
    if (кнопка.hasAttribute("data-cookie-no")) { ответить("no"); }
  });
})();
